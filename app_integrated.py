...
# Import the chess move predictor
sys.path.append(os.path.join(os.path.dirname(__file__), 'CHESS-MOVE-PREDICTOR'))
try:
    from main import play
    CHESS_PREDICTOR_AVAILABLE = True
    print("✅ Chess move predictor imported successfully")
except ImportError as e:
    CHESS_PREDICTOR_AVAILABLE = False
    print(f"⚠️  Chess move predictor not available: {e}")
...
async def execute_ai_move():
    """Get move from AI predictor and execute it."""
    if not CHESS_PREDICTOR_AVAILABLE:
        print("❌ Error: Chess move predictor not available")
        print("   Make sure the CHESS-MOVE-PREDICTOR folder exists and contains main.py with play() function")
        return
    
    try:
        print("🤖 Getting move from AI predictor...")
        move_dict = play()
        
        # Validate the move dictionary structure
        required_keys = ["piece", "from", "to", "type"]
        if not all(key in move_dict for key in required_keys):
            print(f"❌ Error: Invalid move dictionary from AI. Expected keys: {required_keys}")
            print(f"   Received: {move_dict}")
            return
        
        piece = move_dict["piece"]
        from_position = move_dict["from"].lower()
        to_position = move_dict["to"].lower()
        move_type = move_dict["type"]
        
        print(f"🎯 AI Move Decision:")
        print(f"   Piece: {piece}")
        print(f"   From: {from_position.upper()}")
        print(f"   To: {to_position.upper()}")
        ...
        if not from_valid:
            print(f"❌ Error: AI provided invalid from position - {from_msg}")
            return
            
        to_valid, to_msg = validate_position(to_position)
        if not to_valid:
            print(f"❌ Error: AI provided invalid to position - {to_msg}")
            return
        
        # Check if positions are the same
        if from_position == to_position:
            print("❌ Error: AI provided same from and to positions")
            return
        
        # Execute the move
        await move_chess_piece(from_position, to_position, piece, move_type)
        
    except Exception as e:
        print(f"❌ Error getting move from AI predictor: {e}")
        print("   Check that the play() function in main.py is working correctly")

async def move_chess_piece(from_position, to_position, piece=None, move_type=None):
    """
    ...
    move_description = f"{from_position.upper()} → {to_position.upper()}"
    if piece and move_type:
        move_description = f"{piece} {move_type}: {move_description}"
    
    print(f"🚀 Starting chess move: {move_description}")
    print("=" * 70)
    ...
    try:
        ...
        print(f"\n🎉 Successfully completed chess move: {move_description}")
        print("=" * 70)
...
def print_usage():
    ...
    print("=" * 50)
    print("📋 Usage Options:")
    print("   1. Manual move: python app.py <from_position> <to_position>")
    print("      Example: python app.py d7 d5")
    print("   2. AI predicted move: python app.py --ai")
    print("      Uses chess move predictor to determine next move")
    ...
    if not CHESS_PREDICTOR_AVAILABLE:
        print("\n⚠️  Note: Chess move predictor not available. Only manual moves supported.")
...
async def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1] == "--ai":
        print("🤖 Chess Robot Controller - AI Mode")
        print("=" * 50)
        await execute_ai_move()
        return
    
    # Check for manual mode arguments
    if len(sys.argv) != 3:
    ...
    print("🤖 Chess Robot Controller - Manual Mode")
    print("=" * 50)
